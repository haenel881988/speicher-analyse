# Änderungsprotokoll - Archiv v7.0–v7.2

> Archiviert am 2026-02-10. Aktives Protokoll: [`../aenderungsprotokoll.md`](../aenderungsprotokoll.md)
...
| 27 | 2026-02-10 | v7.2 | fix | **WCAG: var(--accent) als Textfarbe ersetzt** - `var(--accent)` (#6c5ce7, ~4.0:1) wurde überall als Textfarbe durch `var(--accent-text)` ersetzt. 16 Stellen: Breadcrumbs, Quick-Access, Omnibar-Header, Privacy, Netzwerk, Settings, etc. `--accent` verbleibt nur für Rahmen und Hintergründe. |
| 28 | 2026-02-10 | v7.2 | fix | **WCAG: Ordnernamen + accent-text deutlich heller** - Ordnernamen im Explorer verwenden jetzt `--text-primary` (Weiß, ~17:1) statt Akzentfarbe. `--accent-text` von #a78bfa (~7:1, visuell Lila-auf-Blau schlecht wahrnehmbar) auf #c4b5fd (~10.8:1, helles Lavendel) erhöht. WCAG-Kontrastberechnung erfasst nicht alle Hue-Kombinationen korrekt - Lila auf dunklem Navy-Blau hat trotz mathematisch ausreichendem Kontrast schlechte wahrgenommene Lesbarkeit. |
| 29 | 2026-02-10 | v7.2 | improve | **OBERSTE DIREKTIVE: Quellcode-Verantwortung** - Neue unverhandelbare Regel in CLAUDE.md: Jedes Problem liegt IMMER im Quellcode. Niemals den Client/User beschuldigen. Niemals manuelle Workarounds anweisen. Visuelles Ergebnis = Wahrheit. Entstanden aus dem WCAG-Kontrast-Vorfall (5x gemeldet bis gelöst). |
| 30 | 2026-02-10 | v7.2 | fix | **Umfassender Code-Review v1-v7.2** - 6 Fixes: (1) Terminal: `claude` aus PTY-Blockliste entfernt, intelligente Unterscheidung bare/mit-Argumente. (2) Privacy/S.M.A.R.T./Software-Audit: `_loaded`-Flag wird bei Fehler zurückgesetzt → Retry möglich. Retry-Buttons hinzugefügt. (3) Dual-Panel: `getScanId` an rechtes Panel übergeben → Omnibar-Suche funktioniert in beiden Panels. (4) Netzwerk: Auto-Refresh stoppt nach 3 Fehlern (Circuit-Breaker). `_loaded`-Flag zurückgesetzt bei Fehler. (5) Umlaut-Fix: `verfuegbar` → `verfügbar`. (6) Projektplan v7.2 aktualisiert. |
